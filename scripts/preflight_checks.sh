...
if ! command -v nvidia-smi &>/dev/null; then
    ...
    NVIDIA_SMI_OUTPUT=$(nvidia-smi --query-gpu=name,driver_version,memory.total,cuda_version --format=csv,noheader 2>/dev/null || echo "")
    ...
    if [ -z "$NVIDIA_SMI_OUTPUT" ]; then
        ...
        CUDA_DRIVER=$(echo "$NVIDIA_SMI_OUTPUT" | cut -d',' -f4 | xargs)
...
DISK_USAGE=$(df -h /workspace 2>/dev/null || df -h / 2>/dev/null || echo "")
AVAILABLE_GB=$(df -BG /workspace 2>/dev/null | tail -1 | awk '{print $4}' | tr -d 'G' || df -BG / 2>/dev/null | tail -1 | awk '{print $4}' | tr -d 'G' || echo "0")

if [ -z "$DISK_USAGE" ]; then
    warn "Cannot determine disk space"
else
    pass "Disk space:"
    echo "$DISK_USAGE" | head -2 | sed 's/^/    /'
    ...
    if [ "$AVAILABLE_GB" -ge 100 ] 2>/dev/null; then
        ...
        warn "Low disk space (${AVAILABLE_GB}GB) - build may fail"
        warn "Isaac Sim build requires ~100GB"
        CHECK_STATUS["DISK"]=warning
    else
        fail "DISK" "Insufficient disk space (${AVAILABLE_GB}GB available, need 100GB+)"
    fi
fi
